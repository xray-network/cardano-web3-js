...
export class CardanoWeb3 {
  ...
  static init = async (config?: T.InitConfig) => {
    ...
    cw3.libs.CML = await import("@dcspark/cardano-multiplatform-lib-nodejs")
    cw3.libs.MSL = await import("@emurgo/cardano-message-signing-nodejs")
    cw3.libs.UPLC = await import("uplc-node")
    cw3.libs.PlutusData = Data(cw3)
    cw3.libs.PlutusConstr = Constr
    cw3.provider = config?.provider || new KoiosProvider(`https://graph.xray.app/output/koios/${network}/api/v1`)
    cw3.libs.Message = Message(cw3)
